 ...
 * @Desc
 **/
class GankListVewModel  : BaseViewModel() {
    ...
    override fun stop() {
        super.stop()
       listResquest.mSub?.cancel()
    }

    fun loadData(category: String,pageIndex: Int){
        RetroFitHolder.gankInstance().category(category,pageIndex,listResquest)
    }


    private val listResquest = object : BaseGankSubscriber<CategoryResponse>(){
        override fun start() {
            super.start()
